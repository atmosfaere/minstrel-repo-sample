...
`;
    ...
    socket.on('ai stream', function (data) {
    ...
    socket.on('processing message', function () {
    ...
    socket.on('received message busy', function () {
        //if streaming = true on server for room, then message is sent here, send only to client that sent prompt
        //important, user may send messages in same interval, or in case one user doesn't receive processing message that sets isStreaming to true due to connection issue
        //make button normal again
        console.log("server busy processing another room message")
    });

    socket.on('get conversation', function (data) {
